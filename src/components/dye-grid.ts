...
export class DyeGrid extends BaseComponent {
    ...
    render(): void {
        const wrapper = this.createElement('div', {
            className: 'grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 max-h-96 overflow-y-auto',
            attributes: {
                role: 'grid',
        ...
        if (this.dyes.length === 0 && this.emptyState) {
            // Render empty state
            const emptyHtml = this.emptyState.type === 'search'
                ? getEmptyStateHTML({
                    icon: ICON_SEARCH,
                    title: LanguageService.tInterpolate('dyeSelector.noResults', { query: this.emptyState.query || '' }) || `No dyes match "${this.emptyState.query}"`,
                    description: LanguageService.t('dyeSelector.noResultsHint') || 'Try checking your spelling or search for a category like "purple".',
                })
                : getEmptyStateHTML({
                    icon: ICON_PALETTE,
            ...
            wrapper.classList.remove('grid', 'grid-cols-1', 'sm:grid-cols-2', 'md:grid-cols-3', 'lg:grid-cols-4', 'gap-3');
